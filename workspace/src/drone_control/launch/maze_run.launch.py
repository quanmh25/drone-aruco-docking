...
def generate_launch_description():
    ...
    sim_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_sim, 'launch', 'maze_sim.launch.py')
        )
    )
    ...
    maze_docking_node = Node(
        package="drone_control",
        executable="maze_docking_node",
        name="maze_docking_node",
        output="screen"
    )

    return LaunchDescription([
        sim_launch,
        TimerAction(period=7.0, actions=[maze_docking_node]),
        # rqt_image_view_node,
    ])
